...
**ArborInversa（逆生树）** 是一个**树型思维导图**应用，也是承载内容的**通用层级框架**。
其下可挂接若干棵**分支树**，当前已生长出的分支树是**中国美术史**。

### 名称来源与沿革

- **ArborInversa** 为项目的**统一正式名**（拉丁名），出自 **赫尔墨斯卡巴拉**
  （Hermetic Qabalah）中的「倒生之树」。
- 中文名 **逆生树**。⚠️ **曾用名「倒生树」**——旧文档／旧会话中出现「倒生树」均指本项目，
  现已全部统一为 `ArborInversa`（英文/文件名）与「逆生树」（中文）。
- ★ **「逆生树」就是 `ArborInversa` 的中文名**——两者是同一项目的两种写法
  （中文／拉丁），**不是两个项目**；界面、文档中可互换称呼。
- 命名呈现：
  - 程序窗口标题：`ArborInversa`
  - 页面标题（title）：`ArborInversa`
  - 页面左上角：`ArborInversa` ＋ 副标题 `逆生树`
  - 可执行文件／目录：`C:\ArborInversa\ArborInversa.exe`
  - 桌面快捷方式：`ArborInversa`

### 卷首题词

界面**描述区右下角**显示此题词（`index.html` 中 `.epigraph`；**加粗、字号为原字号的 1.5 倍**
——主句 17.25px、出处 15.75px）：
...
**根在上，枝向下**——越往下越具体，越往上越总括，如倒生之木；
**叶**则是挂在枝上的颂歌（描述），故题词说「叶为颂歌」。

### ★ 核心概念（术语以此为准，务必记住）

| 术语 | 方向 | 含义 |
|---|---|---|
| **枝** | **竖向** | 目录树中的层级节点。向下生长子枝，构成树形骨架 |
| **叶** | **横向** | 挂在枝上的**描述板块**。**一枝可有多片叶**，叶名显示在枝名右侧 |
...
```
两宋  ──🍃形势 ──🍃变迁 ──🍃环        （叶名显示在枝名右侧，点击切换）
```

- 理念：**一切皆可为树**——任意枝都可继续往下生长子枝，同时任意枝都可横向展叶；
  **任意学科／项目／资料体系都可挂为一棵分支树**。
- 形态：可整体打包带走的**独立软件**（见第二节）。
...
| `data.json` | **已冻结的旧快照**（2026-09-14 11:25，不再随内容更新，见下节） |
| `media/` | 图片源 |
| `serve.js` | 备用 Node 后端 |
...
| `data.backup.json`<br>`data.before-*.json` | **历史备份**（10 个，见第十一节回退方法） |
...
- 层级约定：
  - **根** ＝ 逆生树（框架本体）
  - **根的直接子枝** ＝ **分支树**（当前只有「中国美术史」；未来可平行增加其它学科）
  - **分支树的子枝** ＝ 该学科的时代／分类
...
### ★ 属性（props）与统一规则（2026-09-14 统一补全）

- 现状：**122 个枝全部带属性，共 450 条**（生成脚本 `tools/seed_props.js`，可随时复跑）

| 键 | 条数 | 生成规则 |
|---|---|---|
| `类型` | 122 | 结构位置 ＋ 名称 ＋ 正文关键词：框架／分支树／时代／分类／门类／题材／画家／书法家／书画家／流派／窑口／遗址／画论／文物／文化／现象 |
| `朝代` | 120 | 该枝所属时代（取第 2 层祖先）；「史前与三代」再细分为 新石器时代／商周 |
| `门类` | 87 | 路径上的 绘画／书法／文物；画家补「绘画」、书法家补「书法」、书画家补两者 |
| `题材` | 60 | 路径上的 人物／山水／花鸟／道释／肖像／风俗／历史；画派成员按画派补（元四家等 ＝ 山水） |
| `别名` | 41 | 画家／书法家／书画家：从正文抽 **字**、**号**、**人称/世称『…』**、**原名**；风格名不入（如「关家山水」不算别名） |
| `年代` | 10 | 只给 10 个时代枝写（历史年代区间，如 两宋 ＝ 960—1279） |
| `流派` | 10 | 路径上的群体枝（元四家／南宋四家／李成传派／明四家／四王吴恽／扬州八怪／海派） |

- **值类型铁律**：单值写成**文本**，多值写成**数组**；**不要写单元素数组**（编解码会把它还原成文本，
  自测里有这条断言）
- **复跑**：`node tools/seed_props.js`（空跑，只打印将要写入的属性）／`node tools/seed_props.js --write`
  （写盘；首次自动备份 `data.before-props.json`）。脚本**只补自己负责的键，手写属性不会被抹掉**
- 给别的时代补完枝干后，**复跑一次**即可让新枝也带上属性（外发版与试用代码不受影响）
...
- **枝（节点）总数 122**，**叶总数 142**，图片引用 1 处
- 跳转链接 **43 处，全部有效**
- 结构：逆生树 → 中国美术史（分支树）→ 10 个时代
...
- **属性已统一补好**：122 个枝共 450 条（`类型`／`朝代`／`门类`／`题材`／`别名`／`年代`／`流派`），
  所以属性维度**开箱可用**，例如「在枝下＝元代 ＋ 属性 类型＝画家」；
  键与规则见第四节「属性（props）与统一规则」
...
于是「根本来是空的 → 导入美术史那棵树 → 就成现在这个样子」这条路是通的：
...
## 七、内容写作规范（用户确立的标准，务必遵循）

### 1. 时代（分支树下的时代枝）标配三片叶

| 叶名 | 应写什么 |
|---|---|
| **形势** | 当时的**形势、环境、思潮**：政治格局、社会结构、思想潮流、制度机构。**按点分段**（用空行分隔） |
| **变迁** | 相比**前代**产生了什么变化：环境与思潮之变、技法／技术／理论之发展。分段 |
| **环** | 该时代**作为历史环节**的特殊性与贡献——它在整条脉络中承担什么角色。收束性判断 |

> 三片叶是**时代这一层级的标准配置**，10 个时代均已配备。

### 2. 母枝（含子枝的枝）要详写

含子枝的母枝，描述要写**时代背景、思想风潮、社会观念、制度沿革**，不能只写一句概述。

### 3. 末端叶用「考研美术史名词解释」体

统一结构：**身份／字号 → 师承与创法 → 风格特点 → 代表作 → 画史地位**。
例（范宽）：

> 北宋山水画家，名中正，字中立，华原人，因性宽厚人称『范宽』。隐居终南、太华，以造化为师，
> 创『雨点皴』，山峦浑厚雄壮，《溪山行旅图》为全景山水巅峰……

- 引号统一用 `『』`（书名／术语）
- 分段用空行
- 描述中提到的**已有枝名**应写成 `[[…]]` 以便跳转
...
| 时代 | 枝数（含自身） | 枝干状态 | 三叶 | 备注 |
|---|---|---|---|---|
| 史前与三代 | 7 | 有骨架 | ✅ | |
| 秦汉 | 4 | 简单 | ✅ | 枝干待梳理 |
| 魏晋南北朝 | 4 | 简单 | ✅ | 枝干待梳理 |
| 隋唐 | 4 | 简单 | ✅ | 枝干待梳理 |
| **五代** | 15 | 有骨架 | ✅ | 绘画（山水／人物／花鸟）＋ 书法 |
| **两宋** | **59** | **最完整** | ✅ | 绘画（人物／山水／花鸟）＋ 书法 ＋ 文物 |
| **元代** | **16** | **用户亲自搭建** | ✅ | 前期→遗民→入化／不化；中后期→元四家（含 9 位画家） |
| 明 | 3 | 简单 | ✅ | 明四家、徐渭 |
| 清 | 4 | 简单 | ✅ | 八大山人、四王吴恽、扬州八怪 |
| 近现代 | 4 | 简单 | ✅ | 海派、徐悲鸿、当代水墨 |

**用户明确说过**：「其他（除元代／两宋）我还没梳理好枝干」
——**不要擅自给其他时代搭建枝干**，除非用户明确要求。

---

## 九、元代架构（用户亲自设计，作为范式参考）
...
含义：元初画坛的核心议题是汉族士人的「**出处**」（在异族统治下如何自处），
故用「遗民 → 入化／不化」区分态度。这是用户认可的架构范式。
...
| `美术史树-试用.txt` | **由当前数据实时生成**的「中国美术史」整棵树代码（当前 121 枝／141 叶／699 行）；UTF-8 带 BOM、CRLF，记事本可直接打开 |
...
- [ ] 其他时代（秦汉、魏晋南北朝、隋唐、明、清、近现代）的枝干，待用户梳理后补充
- [ ] 五代枝干的深化（当前为用户／AI 起的初步骨架）
- [ ] 逆生树根下挂接**其它分支树**（框架已支持，尚无第二棵）
- [ ] 可选：叶内图片的尺寸／对齐控制（如 `![[图|题注|300]]` 指定宽度、图文并排）
- [x] 应用图标（2026-09-14 完成：`Etz.png` → 7 尺寸 ico，已内嵌 exe）
- [x] **枝属性（`props`，参考 Obsidian Properties）＋ 统一补全 122 枝 450 条 ＋ 详情页属性栏 ＋
...
> 项目是 **ArborInversa（逆生树）**，一个**树型思维导图**——名称出自赫尔墨斯卡巴拉。
> **竖向的是枝，横向的是叶**——一枝可挂多片叶；叶为颂歌（描述）。
> 主交付形态是 Windows 独立应用 `C:\ArborInversa\ArborInversa.exe`（内置 HTTP 服务 + WebView2，
> 不依赖 WSL/Node，可整体拷走）；**内容以 `C:\ArborInversa\data.json` 为准**。
> 源码／文档／备份在 `/home/elu/test/doc-tool/daosheng-tree/`。
> 改内容前先备份；给时代写内容时记得标配「形势／变迁／环」三片叶；不要擅自给其他时代搭枝干。
