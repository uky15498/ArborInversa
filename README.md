...
- **层级**：根 → 中国美术史 → 10 个时代 → 时代内部再按门类／题材／画家／作品分层（当前最深 8 层）
- **规模**（2026-09-14）：枝 **122**、叶 **142**、属性 **448**、`[[跳转]]` 43 处（全部有效）、插图 1 处
...
| 文件 | 是什么 |
|---|---|
| `data.json` | 全部内容（枝、叶、属性） |
| `index.html` | 界面（含全部 CSS／JS，无外部依赖） |
| `media/` | 插图 |
| `ArborInversa.exe` | Windows 主程序（内置 HTTP 服务 ＋ WebView2 窗口） |
| `使用说明.txt` | 给最终用户的说明 |

- **正本**：`C:\ArborInversa\`（可用应用内编辑，也可直接改文件）
- **源码／文档／备份**：`/home/elu/test/doc-tool/daosheng-tree/`（含 `win-app/Program.cs`、`tools/`、`README.md`）
...
| 打包发布 | 一键出绿色版 zip ＋ 安装包（Inno Setup），并附示例树的试用代码 | 第十一节 |
...
> 枝上还可带属性（键值对）。结构、字段、规模见第一节。
> 主交付形态是 Windows 独立应用 `C:\ArborInversa\ArborInversa.exe`（内置 HTTP 服务 ＋ WebView2，
> 不依赖 WSL／Node，可整体拷走）；**内容以 `C:\ArborInversa\data.json` 为准**，源码／文档／备份在
> `/home/elu/test/doc-tool/daosheng-tree/`，远端私有库 `github.com/uky15498/ArborInversa`。
> 改完内容跑 `node tools/snapshot.js` 再提交（提交钩子会跑两套自测）；给时代写内容时标配「形势／变迁／环」三片叶；
> 不要擅自给其他时代搭枝干。
